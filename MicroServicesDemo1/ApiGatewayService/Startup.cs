    ...
    public class Startup
    {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, IHostApplicationLifetime lifetime)
        {
            if (env.IsDevelopment())
            {
                ...
                //var apis = new List<string> { "Service01", "Service02", "Service03" };//这里配置微服务swagger名称
                var apis = new List<string> { "Service01"};//这里配置微服务swagger名称
                app.UseSwagger();
                app.UseSwaggerUI(c => {
                    c.SwaggerEndpoint("/swagger/v1/swagger.json", "ApiGatewayService v1");
                    apis.ForEach(m =>
                    {
                        c.SwaggerEndpoint($"/{m}/v1/swagger.json", $"{m} v1");
                    });
                });
            }
